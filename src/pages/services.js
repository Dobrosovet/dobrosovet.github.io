...
export const ServicesPage = () => `
    <div class="max-w-4xl mx-auto pt-8 pb-16 px-4 page-wrapper">
        <h1 class="text-5xl font-black mb-4 text-slate-900 tracking-tight">Наши услуги</h1>
        <p class="text-xl text-slate-500 mb-12 max-w-2xl">Мы предоставляем широкий спектр правовой помощи Кредиторам-истцам при составлении исковых требований, возражений, апелляций, иных документов.</p>
        
        <div class="flex flex-col gap-4">
            ${services.map(svc => `
                <div class="service-card group cursor-pointer bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-indigo-500/5 transition-all duration-300">
                    <div class="flex items-center gap-6">
                        <div class="p-5 bg-slate-50 rounded-2xl group-hover:bg-indigo-600 transition-colors duration-300 shadow-inner">
                            <i data-lucide="${svc.icon}" class="w-8 h-8 text-slate-600 group-hover:text-white transition-colors duration-300"></i>
                        </div>
                        
                        <div class="flex-1">
                            <h3 class="text-2xl font-bold text-slate-900 mb-1 group-hover:text-indigo-600 transition-colors">${svc.title}</h3>
                            <p class="text-slate-500 text-lg leading-relaxed">${svc.desc}</p>
                        </div>

                        <div class="hidden md:flex opacity-0 group-hover:opacity-100 group-hover:translate-x-2 transition-all duration-300">
        ...
        <div class="mt-16 p-8 bg-indigo-600 rounded-[2.5rem] text-center shadow-2xl shadow-indigo-200">
            <h2 class="text-2xl font-bold text-white mb-4">Не нашли нужную услугу?</h2>
            <p class="text-indigo-100 mb-6">Свяжитесь с нами для индивидуальной консультации.</p>
 <button onclick="navigateTo('contacts')" class="px-8 py-4 bg-white text-indigo-600 rounded-2xl font-bold hover:bg-slate-50 transition-colors shadow-lg shadow-indigo-800/20">
    Связаться с юристом
</button>
        </div>
    </div>
